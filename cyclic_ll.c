...
int has_cycle(node *head) {
  ...
  struct node *ptr2 =head;//declares 2 pointers two head

  while (ptr1 != NULL && ptr2 != NULL && (ptr2->next) != NULL){// while the pointers exist

    ptr1 = ptr1->next; //increment first pointer by one
    ptr2 = ptr2-> next-> next; // increment the second pointer by two

    if(ptr1==ptr2){ // if the two pointers are equal return
