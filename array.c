...
int* bubbleSort(int *arr, int n){
    ...
    for (i=0;i<n-1;i++){
        for(j=0;j<n-1;j++){
            if(*(s_arr+j) > *(s_arr+j+1)){// if arr[j] is greater than arr[j+1] switch them
                temp = *(s_arr+j+1);
                *(s_arr+j+1) = *(s_arr+j);
                *(s_arr+j) = temp;
            }
...
int bSearch(int *arr, int a, int b, int key){
  ...
  /*   while(a <= b){
        ...
        if(a>b)return -1;// if key is not in array return -1

        int half=(a+b)/2; // get half the harr

        if((*(arr+half))==key)return half; // check if half is equal to key

        if((*(arr+half)> key)) return bSearch(arr,a,half-1,key);// if not check if arr[half] is greater than key. If so return boundery b as b-1
        
        return bSearch(arr,half+1,b,key);// if half is less than key make boundery "a" as (half -1)
